...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<BaTestEnvironment-request>)))
  ...
  (cl:format cl:nil "# ROS service for testing sparce Bundle Adjustment~%# parameter for testdata structure~%int32 number_points~%float32 frame_view_number~%float32 cone_points_per_plane ~%~%# perameter to describe errors~%float32 limit_error_matching~%~%float32 ba_kernel_param~%int32 ba_num_iter~%float32 ba_initial_mu~%float32 ba_final_mu_factor~%float32 ba_tau~%~%float32 angle_sigma_degree~%float32 translate_sigma_m~%float32 obs_point_sigma_m~%float32 world_point_sigma_m~%~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'BaTestEnvironment-request)))
  "Returns full string definition for message of type 'BaTestEnvironment-request"
  (cl:format cl:nil "# ROS service for testing sparce Bundle Adjustment~%# parameter for testdata structure~%int32 number_points~%float32 frame_view_number~%float32 cone_points_per_plane ~%~%# perameter to describe errors~%float32 limit_error_matching~%~%float32 ba_kernel_param~%int32 ba_num_iter~%float32 ba_initial_mu~%float32 ba_final_mu_factor~%float32 ba_tau~%~%float32 angle_sigma_degree~%float32 translate_sigma_m~%float32 obs_point_sigma_m~%float32 world_point_sigma_m~%~%~%~%"))
